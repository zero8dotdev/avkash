...
`user` stays the Better Auth identity + the _operational_ fields the app reads (role, teamId,
...
- **Offboarding (MVP)** — HR sets status + `exitDate`/`exitReason`. _Follow-ups (v2):_ auto-encash
...
1. **`reportingManagerId`** — a _direct_ manager pointer on the profile, **in addition to**
