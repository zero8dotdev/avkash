...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        IF NEW.keyword = 'joined' THEN
            INSERT INTO public."ActivityLog" ("tableName", "userId", "changedColumns", "oldValues", "newValues", "changedBy", keyword)
            VALUES (tableName, NEW."userId", NULL, NULL::jsonb, row_to_json(NEW)::jsonb, NEW."updatedBy", 'joined');
        ELSE
            INSERT INTO public."ActivityLog" ("tableName", "userId", "changedColumns", "oldValues", "newValues", "changedBy", keyword)
            VALUES (tableName, NEW."userId", NULL, NULL::jsonb, row_to_json(NEW)::jsonb, NEW."updatedBy", 'invitation');
        END IF;
    END IF;
...
-- Trigger to call the function after an insert on the User table
