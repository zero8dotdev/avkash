...
const VALID: ReadonlySet<string> = new Set<Day>(['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'])
...
export async function setUserWorkweek(ctx: AuthContext, userId: string, days: string[]): Promise<User> {
  ...
  if (ctx.role !== 'OWNER' && ctx.role !== 'ADMIN') {
    ...
    let isManager = false
    if (target.teamId) {
      const [t] = await db.select({ managers: schema.team.managers }).from(schema.team).where(eq(schema.team.teamId, target.teamId)).limit(1)
