...
import { type AuthContext, NotFoundError, BusinessRuleError, ORG_GRAPH_EVENTS } from '@avkash/shared';
...
export async function approveTransfer(ctx: AuthContext, transferId: string) {
  ...
  try {
    ...
    console.error('[authz-sync] publish employee.transfer.completed failed:', err instanceof Error ? err.message : err);
  }
  return row;
}
