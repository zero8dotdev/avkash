...
export async function utilization(ctx: AuthContext, opts?: { teamId?: string; year?: number }): Promise<UtilizationRow[]> {
  requireRole(ctx, 'MANAGER')
  ...
  return rows.map((r) => ({ leaveTypeId: r.leaveTypeId, name: r.name, taken: Number(r.taken), planned: Number(r.planned) }))
}
