...
export async function listLeaveComments(ctx: AuthContext, leaveId: string): Promise<LeaveComment[]> {
  ...
  return db
    .select()
    .from(schema.leaveComment)
    .where(and(...conds))
    .orderBy(asc(schema.leaveComment.createdAt));
}
...
async function notify(
  ...
  if (comment.visibility === 'SHARED' && authorId !== lv.userId) {
    const [applicant] = await db
      ...
      .from(schema.user)
      .where(eq(schema.user.id, lv.userId))
      .limit(1);
  ...
  if (comment.visibility === 'INTERNAL' || authorId === lv.userId) {
    const [t] = await db
      ...
      .from(schema.team)
      .where(eq(schema.team.teamId, lv.teamId))
      .limit(1);
    ...
    if (managerIds.length) {
      const mgrs = await db
        .select({ email: schema.user.email })
        .from(schema.user)
        .where(inArray(schema.user.id, managerIds));
      for (const m of mgrs) recipients.add(m.email);
    }
  }
  for (const email of recipients)
    void sendEmail({ to: email, subject: 'New comment on a leave request', text: comment.body });
}
