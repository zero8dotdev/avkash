...
import { Button, Card, Checkbox, Col, Flex, Form, List, Row, Switch, Typography } from "antd";
import { LeftOutlined } from "@ant-design/icons";
import { useRouter } from "next/navigation";
import TopSteps from "../_componenets/steps";
import { fetchTeamGeneralData, updateInitialsetupState, updateTeamNotificationsSettings } from "../_actions";
...
const Notifications = () => {
  ...
  const { data: team, error, mutate, isValidating: teamLoading } = useSWR(
  ...
  const handlenext = async () => {
    ...
    try {
      ...
      if (status) {
        router.push(new URL("/initialsetup/invite-users", window?.location.origin).toString());
      } else {
  ...
  const handlePrevious = () => {
    router.push(new URL("/initialsetup/locations", window?.location.origin).toString());
  };
