...
const upsertSubscription = async (subscription: any) => {
  ...
  const { data, error } = await supabaseAdmin.from('Subscription').upsert({
    id,
    entity,
    planId: plan_id,
...
const fetchSubscription = async (subscriptionId: any) => {
  try {
    ...
    const response = await fetch(
      `${process.env.RAZORPAY_URL}subscriptions/${subscriptionId}`,
      {
        method: 'GET',
        headers: {
...
export const POST = async (req: NextRequest) => {
  ...
  try {
    ...
    if (relevantEvents.has(event.event)) {
      ...
      return NextResponse.json(
        {
          error: `Unsupported event type: ${event.event}`,
        },
        { status: 400 }
      );
    }
