...
export async function handleApproveReviewLeave(
  ...
  const startDate = view?.state?.values?.start_date_block?.start_date
    ?.selected_date;
  const endDate = view?.state?.values?.end_date_block?.end_date?.selected_date;
  const duration_temp = view?.state?.values?.day_type_block?.day_type
  ...
  const leaveType = view?.state?.values?.type_block?.type?.selected_option
    ?.value;
