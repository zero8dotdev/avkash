...
import {
  ...
  getLeaveTypeDetails,
  fetchIsHalfDay,
} from '../header/_components/actions';
...
export async function createCommonModalBlocks({
  avkashUserInfo,
  checkLeaveType,
  leaveId,
  payload,
  ...
  let shift: string = '';
  const [leavesList, leaveTypesList, workWeekData, isHalfDayAllowed]: [
    any,
    any,
    WorkWeekDataProps | null,
    boolean,
  ] = await Promise.all([
  ...
  const initial_shift_value = {
    text: {
      type: 'plain_text',
      text: shift.charAt(0).toUpperCase() + shift.slice(1),
    },
    value: shift,
  };
  ...
  const leaveTypes =
    leaveTypesList &&
    leaveTypesList.map((leave: { leaveTypeId: String; name: string }) => {
      const res = {
        text: {
          type: 'plain_text',
          text: leave.name,
  ...
  if (checkLeaveType && payload) {
    ...
    const selectedUserId = values.select_user_block
      ? values.select_user_block.select_user.selected_option.value
      : avkashUserInfo.userId;
    ...
    const [res, count, isLeaveInAccured]: [string, number, boolean] =
      await calculateWorkingDays(
        ...
        leaveType,
        workWeekData,
        userDetails.accruedLeave,
        userDetails.usedLeave
      );
    blocks.splice(4, 0, {
      type: 'context',
      elements: [
...
export async function calculateWorkingDays(
  ...
  endDate: string,
  leaveType: string,
  workWeekData: WorkWeekDataProps | null,
  accruedLeave?: any,
  usedLeave?: any
  ...
  const [holidays, fetchLeaveTypes]: [any, any] = await Promise.all([
    fetchHolidays(startDate, endDate, location),
    getLeaveTypeDetails(leaveType, orgId),
  ]);
