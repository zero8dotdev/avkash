...
export default async function handleAppHomeOpened({
  avkashUserInfo,
  yourDashboard,
  viewId,
  ownerSelectedTeamId,
  ...
  if (avkashUserInfo.isOwner && !yourDashboard) {
    ...
    let teamId;
    teamId = ownerSelectedTeamId || avkashUserInfo.teamId;
    const [allLeavesHistory, pendingHistory] = await fetchLeavesHistory({
      days: 7,
      teamId,
    });
  ...
  const common_top_block = createButtonsBlock([
    ...
    {
      text: 'Web App',
      action_id: 'home-web-app',
      url: 'https://www.avkash.io/',
    },
  ...
  const teamDashboardBlock = () => {
    if (avkashUserInfo.isOwner) {
      ...
      const initialOption = options.find(
        (option: { value: string }) => option.value === avkashUserInfo.teamId
      );
      return [
        {
          type: 'actions',
  ...
  const pendingLeavesBlocks: any =
    !isManager || yourDashboard
      ? userUpcomingBlock(pendingLeaves)
      : createPendingLeaveBlocks(pendingLeaves);
  const checkOwnerOrManagerOrUser = avkashUserInfo.isOwner
    ? [{ type: 'divider' }, ...teamDashboardBlockArray]
    : isManager
      ? [{ type: 'divider' }, teamDashboardBlockArray]
      : [];
  const blocks = [
...
export async function fetchLeavesHistory({
  days,
  userId,
  teamId,
}: {
...
function createButtonsBlock(
  buttons: { text: string; action_id: string; url?: string }[]
) {
  return {
    type: 'actions',
...
function createLeaveHistoryBlock(
  leaves: {
    leaveId: string;
    leaveType: any;
    startDate: any;
  ...
  const noPendingLeaves = leaves.filter(
...
function createPendingLeaveBlocks(
  pendingLeaves: {
    leaveId: string;
    startDate: string;
    endDate: string;
    ...
    blocks: [
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `Welcome to your Home tab, <@${slackId}>!`,
        },
      },
      { type: 'divider' },
      {
        ...
        text: {
          type: 'mrkdwn',
          text: 'check below to know what you can do with Avkash!!',
        },
      },
      ...blocks,
    ],
