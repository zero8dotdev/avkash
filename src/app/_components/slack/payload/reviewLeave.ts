...
export default async function reviewLeave(
  ...
  const openedReviewModalViewId = await openView(
    avkashUserInfo,
    trigger_id,
    loadingView
  );
  const [leavesList, commonBlocks] = await Promise.all([
    getLeaveDetails(leaveId),
    createCommonModalBlocks({
      avkashUserInfo,
