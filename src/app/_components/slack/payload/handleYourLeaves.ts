...
export default async function handleYourLeaves(
  avkashUserInfo: avkashUserInfoProps,
  channelId: string
) {
  const { userId } = avkashUserInfo;
  const [leavesAllData, workWeekData] = await Promise.all([
    fetchLeavesHistory({ days: 30, userId }),
    fetchOrgWorkWeek(avkashUserInfo.orgId),
  ]);
  const [allLeavesHistory, pendingHistory] = leavesAllData;

  const leavesHistory = await Promise.all(
    allLeavesHistory.map(async (leave) => {
