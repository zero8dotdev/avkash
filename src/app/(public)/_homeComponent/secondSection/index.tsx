  ...
  <div className="lg:grid lg:grid-cols-12 lg:justify-end w-full md:rounded-t-xl bg-white/10 ring-1 lg:ring-0 ring-white/10 ring-inset pb-6 lg:bg-transparent pt-4 md:pt-6 md:pb-0 md:rounded-b-lg">
    <div className="lg:col-span-5 xl:col-span-4 xl:col-start-2 mt-6 my-2 flex lg:flex-col  gap-x-6 md:gap-x-8 px-4 md:px-6 lg:px-0 py-4 justify-center md:justify-center items-center overflow-x-auto sm:overflow-visible text-[#2563ea] scrollbar-hide mx-auto lg:mx-0">
      {tabs.map((tab, index) => (
        <div
          key={index}
          className={`flex-shrink-0 lg:px-4 lg:py-2 lg:my-3 border border-transparent cursor-pointer
        ...
        >
          ...
          <p className="hidden lg:block text-md font-light text-white/90 lg:w-full xl:tracking-wide xl:leading-7">
            {tab.description}
          </p>
        </div>
      ))}
    </div>
    <p className="block lg:hidden text-md md:text-lg lg:text-md font-light text-white/90 text-center tracking-normal px-2 md:px-4 mb-8">
      {tabs[activeTab].description}
    </p>
    <div className="flex justify-center w-full lg:col-span-7 overflow-hidden bg-white border rounded-md">
      {tabs[activeTab] && (
