...
const tabData = [
  {
    ...
    detail: 'We talked about reporting in the section above but we needed three items here, so mentioning it one more time for posterity.',
    imgSrc: '/screen4.png'
  },
  {
    title: 'Reporting',
...
export const ThirdSection = () => {
  ...
  return (
    <div className='max-w-6xl mx-auto'>
      ...
      <div ref={containerRef} className='w-full flex overflow-x-auto scroll-smooth snap-x no-scrollbar p-12 bg-gray-300 gap-10 rounded-xl'>
        {tabData.map((tab, index) => (
          <div
            key={index}
          ...
          >
            <img src={tab.imgSrc} alt={`screen ${index}`} className='w-full rounded-xl' />
          </div>
        ))}
      </div>
    </div>
