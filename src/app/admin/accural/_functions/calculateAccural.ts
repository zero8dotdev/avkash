...
export const calculateAccural = async (frequency: IFrequency, accrueOn: IAccuredOn) => {
  try {
    ...
    await Promise.all(policies.map(async (policy) => {
      ...
      await Promise.all(orgUsers.map(async (user) => {
        let { userId, accruedLeave } = user;
        if (accruedLeave && accruedLeave !== null) {
          accruedLeave = Object
            .keys(accruedLeave)
            .reduce((accumulator: { [key: string]: any }, _policyId: string) => {
              const policy = accruedLeave[_policyId];
              accumulator[_policyId] = { ...policy, balance: policy.balance + accuralCount };
              return accumulator;
            }, {});
